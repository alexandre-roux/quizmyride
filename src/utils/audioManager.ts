...
const soundsRel: Record<string, string> = {
  yay: 'sounds/yay.mp3',
...
function createAudio(src: string, volume = 0.8) {
...
export function getAudio(key: string): HTMLAudioElement | null {
...
export function play(key: string, { fromStart = true }: { fromStart?: boolean } = {}) {
  ...
  if (p && typeof (p as any).catch === 'function') {
    return (p as Promise<void>).catch((err) => {
      ...
      const detail = { key, src: audio && audio.src, errorMessage: (err && (err as any).message) || String(err) }
      try {
...
export function warmUp() {
  ...
  try {
    ...
    if (pr && typeof (pr as any).then === 'function') {
      return (pr as Promise<void>)
        .then(() => {
          stop()
        })
        .catch((err) => {
          const detail = { key: 'gong', src: a && a.src, errorMessage: (err && (err as any).message) || String(err) }
          try {
